...
| Cosmos 和 Runway 远程媒体保留 | `generate`、`transfer` | WorldForge 现有的 Cosmos 和 Runway HTTP 适配器 | 宿主方持有凭据、端点可达性和持久化媒体工件 | 解析器夹具、失败轮询夹具、过期/不支持工件夹具、签名 URL 脱敏测试 | 通过远程媒体空运行和净化后的运行清单，在已准备好的宿主上具有可信度 | 低至中等：当前适配器已处于测试阶段；风险在于工件保留和 URL 泄漏，而非新的运行时形态 | 12 | 活跃批次。通过 [#134](https://github.com/AbdelStark/worldforge/issues/134) 强化当前提供方；暂不新增其他远程视频 API。 |
...
| 空间/3D 场景提供方族群 | 在场景工件契约确立后是未来的 `generate` | 无单一选定 API；OpenLRM/I-Scene 风格项目仅为研究信号 | 宿主方将持有 GPU/运行时包、模型权重、资产保留、查看器和单位约定 | 尚未就绪；首先需要 JSON 原生场景工件夹具和格式错误载荷覆盖 | 在选定一个具体 API 和工件模式之前不具可信度 | 中至高：快速迭代的项目、大型工件、不明确的场景单位语义 | 7 | 延期。在实现前通过 [#138](https://github.com/AbdelStark/worldforge/issues/138) 重新评估。 |
| Genie 交互式世界生成 | 仅在支持的运行时/API 存在后是未来的 `generate` | 在当前项目文档/搜索中未找到支持的自动化 API 或官方可调用运行时；当前 `genie` 保持为脚手架 | 如果出现真实契约，宿主方将持有凭据/运行时/工件保留 | 仅脚手架测试；在上游契约选定之前没有真实的解析器夹具 | 目前不具可信度 | 高：上游契约不明确，过度声明风险高 | 5 | 延期。在存在具体的运行时或 API 契约之前，保持 `genie` 失败关闭。 |
| 其他远程视频 API | `generate` 或 `transfer` | 未选定新 API | 宿主方将持有凭据、工件保留和提供方专用轮询/下载策略 | 在 API 选定后可固定夹具，但当前价值与 Cosmos/Runway 强化重复 | 仅在提供方专用工件策略已知后具可信度 | 中等：签名 URL、保留和提供方更迭风险 | 8 | 延期。在 [#134](https://github.com/AbdelStark/worldforge/issues/134) 完成之前不新增其他远程媒体提供方。 |
| 仿真器桥接 | 未来的 `predict`、`generate` 或宿主工作流（视桥接类型而定） | 未选定仿真器桥接契约 | 宿主方持有仿真器进程、资产、控制器、安全策略和持久化存储 | 尚未就绪；首先需要场景/状态边界和宿主方持有的进程模型 | 在状态/工件模式存在之前，作为提供方冒烟测试不具可信度 | 高：仿真器设置和控制器假设可能泄漏到核心中 | 6 | 延期。在空间/场景边界和状态工件夹具存在后重新评估。 |
...
2. **Cosmos/Runway 远程媒体保留强化。** 在新增任何远程视频 API 之前完成 [#134](https://github.com/AbdelStark/worldforge/issues/134)。重点是工件生命周期、过期/不支持媒体处理、安全的本地摘要/路径、重试耗尽和签名 URL 脱敏。
3. **Nano World Model 打分候选侦察。** 使用 [#158](https://github.com/AbdelStark/worldforge/issues/158) 进行宿主方持有的可选运行时契约设计。在存在真实可调用的打分边界、夹具、事件脱敏和已准备好宿主的冒烟路径之前，它不是提供方目录条目。
...
| 候选者 | 具体阻碍 | 重新评估触发条件 |
| --- | --- | --- |
| Genie 运行时/API | 未找到 WorldForge 可封装的受支持自动化 API 或官方运行时契约的文档。 | 一个受维护的上游运行时或托管 API，以有许可证、输入、输出和冒烟证据的可调用工件生成契约形式出现。 |
| 空间/3D 场景提供方 | WorldForge 尚不具备类型化场景工件边界的验证器，也未选定一个提供方 API。 | [空间场景工件边界](./spatial-scene-artifact-boundary.md) 定义了 JSON 原生边界；[#143](https://github.com/AbdelStark/worldforge/issues/143) 在提供方工作开始前添加夹具/验证。 |
| 其他远程视频 API | 当前的生产风险在于现有测试阶段适配器的工件保留和脱敏，而非广度扩展。 | [#134](https://github.com/AbdelStark/worldforge/issues/134) 完成，且新 API 提供不同的能力或用户工作流。 |
...
| NanoWM `predict` 或 `generate` 接口 | 打分是唯一合理的首个契约。视觉滚动尚未被类型化为 WorldForge 预测载荷或媒体工件。 | 打分路径已被证明，且独立设计记录了预测/工件形态、保真度限制和验证夹具。 |
...
3. 完成 [#134](https://github.com/AbdelStark/worldforge/issues/134)，因为它强化现有的测试阶段媒体提供方，并同样解锁 [#144](https://github.com/AbdelStark/worldforge/issues/144)。
4. 仅在 [#133](https://github.com/AbdelStark/worldforge/issues/133) 建立证据后完成 [#137](https://github.com/AbdelStark/worldforge/issues/137)。
5. 在 [#158](https://github.com/AbdelStark/worldforge/issues/158) 证明可调用的打分边界之前，将其视为设计/侦察议题。
6. 在上述阻碍项解除之前，不启动 [#138](https://github.com/AbdelStark/worldforge/issues/138)、[#139](https://github.com/AbdelStark/worldforge/issues/139)、[#143](https://github.com/AbdelStark/worldforge/issues/143) 或新的策略/提供方扩展。
