...
| Spatial/3D scene provider | WorldForge does not yet have validators for the typed scene artifact boundary or one selected provider API. | [Spatial Scene Artifact Boundary](./spatial-scene-artifact-boundary.md) defines the JSON-native boundary; [#143](https://github.com/AbdelStark/worldforge/issues/143) adds fixtures/validation before provider work. |
