...
| `gr00t` | `policy` | Saved GR00T PolicyClient replay through the real provider adapter, named tensor validation for `eef_9d`, `gripper_position`, and `joint_position`, action translation, provider events, and a sanitized replay artifact. |
...
For the GR00T replay, run `uv run --extra harness worldforge-harness --flow gr00t`. A good run
reports `validated_tensors: eef_9d, gripper_position, joint_position`, `translated_actions: 4`, and
`saved_replay_artifact: artifacts/gr00t-policy-replay.json`. The committed replay shape stores
redacted observation summaries and sanitized action tensors only; it does not contain raw
observations, private endpoints, GPU logs, checkpoints, or downloaded model files. The boundary is
intentional: a prepared GPU host validates the live GR00T server shape, while this harness flow
keeps the WorldForge provider contract inspectable after that host is offline.
