...
| [`cosmos`](providers/cosmos.md) | `prepared-host` | `COSMOS_BASE_URL` | `NVIDIA_API_KEY` | 无 | 生成的视频字节<br>工件保留路径 | 健康检查 10s x3；请求 300s x1；轮询 30s x3；下载 300s x3 | `uv run worldforge provider health cosmos` |
...
| [`runway`](providers/runway.md) | `prepared-host` | `RUNWAYML_API_SECRET` 或 `RUNWAY_API_SECRET` | `RUNWAYML_BASE_URL`、`RUNWAYML_ALLOW_LOCAL_ARTIFACT_URLS`、`RUNWAYML_RESOLVE_ARTIFACT_DNS` | 无 | Runway 任务 ID<br>已下载的视频工件<br>工件保留路径 | 健康检查 10s x3；请求 120s x1；轮询 30s x3；下载 120s x3 | `uv run worldforge provider health runway` |
...
| `cosmos` | `COSMOS_BASE_URL=https://cosmos.example uv run worldforge-smoke-cosmos --run-manifest .worldforge/runs/cosmos-live/run_manifest.json` | `NVIDIA_API_KEY` | 宿主方提供可达的 Cosmos 部署和可选的 `NVIDIA_API_KEY` |
...
| `runway` | `RUNWAYML_API_SECRET=<secret> uv run worldforge-smoke-runway --run-manifest .worldforge/runs/runway-live/run_manifest.json` | `RUNWAYML_API_SECRET`、`RUNWAY_API_SECRET` | 宿主方提供 Runway 凭据并持久化返回的工件 |
