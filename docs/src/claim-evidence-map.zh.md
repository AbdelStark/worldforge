...
| `generate` 生成媒体工件。 | `checkout-tested`；`fixture-tested`；远程 API 使用 `prepared-host smoke-tested` | `tests/test_cosmos_provider.py`、`tests/test_runway_provider.py`、`tests/test_remote_video_providers.py` | 在已配置宿主机上运行 `uv run worldforge benchmark --preset remote-media-dryrun` | 返回的媒体属于工件契约，而非质量或安全主张。 |
| `transfer` 转换媒体工件。 | `checkout-tested`；`fixture-tested` | `tests/test_remote_video_providers.py`、`tests/test_provider_contracts.py`、`src/worldforge/testing/fixtures/transfer/` | `uv run worldforge benchmark --provider mock --operation transfer --input-file examples/benchmark-inputs.json` | 远程 transfer 需要提供方凭据以及宿主方持有的工件。 |
| `reason` 和 `embed` 是仅由 mock 支持的窄能力接口。 | `checkout-tested` | `tests/test_provider_contracts.py`、`tests/test_capability_fixtures.py`、`tests/test_benchmark.py` | `uv run worldforge benchmark --preset parser-overhead` | 它们是契约和适配器路径检查，而非通用 LLM 或嵌入质量主张。 |
...
| Cosmos 和 Runway 是远程媒体适配器。 | `fixture-tested`；已配置时使用 `prepared-host smoke-tested` | `tests/fixtures/providers/cosmos_*.json`、`tests/fixtures/providers/runway_*.json`、提供方文档 | `uv run worldforge benchmark --preset remote-media-dryrun` | 凭据、上游可用性、返回工件的保留和付费使用由宿主方持有。 |
...
| TheWorldHarness 是可选的，且与 Textual 隔离。 | `checkout-tested`；`release-gated` | `tests/test_harness_flows.py`、`tests/test_harness_cli.py`、`tests/test_harness_tui.py` | `uv run --extra harness worldforge-harness` | 非 TUI 流程逻辑与 Textual 导入保持独立。 |
