# Jina Provider Workbench Checklist

Status: scaffold

This checklist was generated by `scripts/scaffold_provider.py`. It is not evidence by
itself; it names the contract work required before `jina` becomes executable.

## Scaffold Surface

- Planned capabilities: `embed`
- Provider source: `src/worldforge/providers/jina.py`
- Provider docs: `docs/src/providers/jina.md`
- Placeholder fixtures: `tests/fixtures/providers/jina_*.json`
...
```bash
uv run worldforge provider workbench jina --format markdown
```

## Validation Commands

```bash
uv run pytest tests/test_jina_provider.py
