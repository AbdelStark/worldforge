...
Taxonomy category: Hosted text-embedding API (OpenAI-compatible)
...
- Provider scaffold: `src/worldforge/providers/jina.py`
- Contract tests: `tests/test_jina_provider.py`
- Placeholder fixtures: `tests/fixtures/providers/jina_success.json` and
  `tests/fixtures/providers/jina_error.json`
- Runtime manifest stub:
  `src/worldforge/providers/runtime_manifests/jina.json.stub`
- Workbench checklist: `docs/src/providers/jina-workbench.md`
...
`jina.json` or treated as smoke evidence until every TODO is replaced with a real
...
- [ ] `uv run worldforge provider workbench jina --format markdown` has no failed
