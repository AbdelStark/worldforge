...
| [`voyage`](providers/voyage.md) | `scaffold` | `VOYAGE_API_KEY` | none | none | none | health 10s x3; request 30s x1; poll 30s x3; download 30s x3 | `uv run worldforge provider health voyage` |
...
| `voyage` | `not smoke-testable from WorldForge` | `VOYAGE_API_KEY` | host supplies a Voyage AI API key via the VOYAGE_API_KEY environment variable |
