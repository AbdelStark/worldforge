...
| [`jina`](providers/jina.md) | `scaffold` | `JINA_API_KEY` | none | none | none | health 10s x3; request 30s x1; poll 30s x3; download 30s x3 | `uv run worldforge provider health jina` |
...
| `jina` | `not smoke-testable from WorldForge` | `JINA_API_KEY` | host supplies a Jina AI API key via the JINA_API_KEY environment variable |
