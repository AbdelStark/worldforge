...
def _jina(event_handler: ProviderEventHandler = None) -> BaseProvider:
    from .jina import JinaProvider

    return JinaProvider(event_handler=event_handler)
...
PROVIDER_CATALOG: tuple[ProviderCatalogEntry, ...] = (
    ...
    ProviderCatalogEntry(
        ...
        docs_page="jina.md",
        runtime_ownership=(
            "host supplies a Jina AI API key via the JINA_API_KEY environment variable"
        ),
    ),
)
