...
def _voyage(event_handler: ProviderEventHandler = None) -> BaseProvider:
    from .voyage import VoyageProvider

    return VoyageProvider(event_handler=event_handler)
...
PROVIDER_CATALOG: tuple[ProviderCatalogEntry, ...] = (
    ...
    ProviderCatalogEntry(
        ...
        docs_page="voyage.md",
        runtime_ownership=(
            "host supplies a Voyage AI API key via the VOYAGE_API_KEY environment variable"
        ),
    ),
)
